    ...
    <div class="container py-1  d-block d-sm-none p-2 mt-4">
        ...
        <div class="mobile-services-nav justify-content-center text-center">
            <div class="services">
                ...
                <div class="service">
                    <div href="javascript:void(0)" class="text-center btn mobile-social-btn  @if($loop->iteration==1) for-instagram @elseif($loop->iteration==2) for-youtube @elseif($loop->iteration==3) for-naver @elseif($loop->iteration==4) for-appMarketing  @elseif($loop->iteration==5) for-talk @endif">
                        <div class="icon">
                        ...
                        <div class="accordion">
                            ...
                            <div class="accordion-item">
                                <div class="accordion-header" id="heading{{ $category->id }}">
                                    ...
                                    <button class="accordion-button shadow-none collapsed" style="background-color: {{ $category->color }};color:white" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $category->id }}" aria-expanded="false" aria-controls="collapse{{ $category->id }}">
                                        {{ $category->title ?? '' }}
                                    </button>
                                ...
                                <div id="collapse{{ $category->id }}" class="accordion-collapse collapse" aria-labelledby="heading{{ $category->id }}" data-bs-parent="#accordionExample">
                                    <div class="accordion-body instagram-accordion-body">
                                        <ul class="instagram-likes-ul">
                                            ...
                                            <li style="border-left: 3px solid {{ $subcategory->color }};"><a href="{{ route('front.subcategory_packages' , $subcategory->slug) }}" class="text-decoration-none text-dark">{{ $subcategory->title }}</a>
                                            </li>
                                            @endforeach
                                        </ul>
                                    </div>
    ...
    <div class="container pt-5 d-none d-md-block mb-5">
        <div class="row justify-content-center">
            <div class="col-md-12 col-lg-8 col-xl-8 d-flex justify-content-center text-center">
                @foreach ($services as $service)
                <div class="dropdown {{ !$loop->last? 'me-3': ''  }}" style="width: calc((100% - 7rem) / {{ count($services) }}); aspect-ratio : 1 / 0.9; height:95px;">
                    <div href="javascript:void(0)" class="text-center desktop-social-btn h-100 @if($loop->iteration==1) for-instagram @elseif($loop->iteration==2) for-youtube @elseif($loop->iteration==3) for-naver @elseif($loop->iteration==4) for-appMarketing  @elseif($loop->iteration==5) for-talk @endif">
                        <div class="icon">
                    ...
                    <ul class="dropdown-menu insta-web instagram-pkg-dropdown" aria-labelledby="instaDropDown">
                        @foreach ($service->categories as $category)
                        <li class="instagram-list" onmouseover="changeColors(this, '{{ $category->color ?? 'white' }}', '{{ 'white' }}')" onmouseout="changeColors(this, 'white', '{{ 'black' }}')">
                  ...
                  gap: 0.8rem;">
                            ...
                            <ul class="dropdown-menu dropdown-submenu">
                                ...
                                <li onmouseover="this.style.borderLeft='3px solid {{ $subcategory->color ?? '' }}'" onmouseout="this.style.borderLeft='0'">
                                    <a class="dropdown-item" href="{{ route('front.subcategory_packages' , $subcategory->slug) }}">{{
                                        $subcategory->title ?? '' }}</a>
                                </li>
                                @endforeach
                            </ul>
            ...
            <div class="packages col-xl-6 col-lg-8 justify-content-center text-center">
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    ...
                    <li class="nav-item service-upper-button-li change_title" role="presentation">
                        <button class="nav-link tabs-button service-upper-button  @if($loop->iteration == 1 )active @endif" id="home-tab{{ $loop->iteration }}" data-bs-toggle="tab" data-bs-target="#home_{{ $loop->iteration }}" type="submit" role="tab" aria-controls="home_{{ $loop->iteration }}" aria-selected="true" href="{{ route('front.login') }}">{{ $package->title ?? '' }}</button>
                    </li>
                    @endforeach
                    ...
                    <div class="tab-pane fade @if($loop->iteration == 1 )show active @endif" id="home_{{ $loop->iteration }}" role="tabpanel" aria-labelledby="home-tab{{ $loop->iteration }}">
                        ...
                        <div class="d-flex flex-wrap justify-content-center">
                            ...
                            <?php
                             ...
                             ?>
                            <div data-package-id="{{ $pakage1->id }}" class="package-box bg_orange @if ($loop->iteration==1)active @endif text-center first-box" data-original="{{ $original_price }}" data-sale="{{ $sale_price}}" data-id="{{ $pakage1->id }}" data-title="{{ $pakage1->title ?? '' }}" data-scp="scp{{ $subcategory->id }}" data-qty="{{ $pakage1->qty }}" data-sub-title={{ $pakage1->sub_title }}>
                                <h4 class="fw-bolder">
                                    {{ $pakage1->qty }}
                                </h4>
                                <span>
                                ...
                                <div class="row px-4">
                                    ...
                                    <div class="col-auto col-9">
                                        ...
                                        @if($firstpackage->id==7 || $firstpackage->id===18 || $firstpackage->id==19)
                                            ...
                                            @endphp
                                            @endif
                                            data-value1="수량" data-value2="{{ $qqty }} 증가"
                                             --}}
    ...
    <div class="service-lower-section container d-flex flex-column justify-content-center align-items-center">
        ...
        <div class="d-flex service-lower-lower-section">
            <div class="service-lower-icon d-flex flex-column justify-content-center align-items-center">
                ...
                @else
                <a href="{{ route('front.login') }}"><button class="service-lower-button" type="button">후기작성</button></a>
                @endif
...
<script>
    ...
    $('.first-box').click(function() {
        ...
        if (sale_price != '') {
            ...
            $('.first-box-sale-price1').html('')
            $('.first-box-sale-price1').append(original_price)
        }
